...
```
It is surprising that our expectation was not backed by the results conveyed by the plots above, but we think the type of wine is what must be held accountable for this finding. As mentioned earlier, we should not only note the sweeter the wine the higher the alcohol level, but lighter wines i.e., those with ABV $<= 11.5\%$ tend to exhibit more fresh notes with high level of acidity rather than being more sweet. 

We believe it would be relevant to first see the distribution of alcohol level in both white and red wine, which may shed more light onto understanding why positive correlation was found between Alcohol and residual sugar levels.  

```{r delete}

plot(wine$residual.sugar~wine$alcohol, col='yellow')
plot(redwine$residual.sugar~redwine$alcohol, col='red', add=T)
```
...
white_alc <- hist(wine$alcohol)
red_alc <- hist(redwine$alcohol)

plot(white_alc, col=rgb(1, 1, 0.2), alpha=0.1)
plot(red_alc, col=rgb(0.5, 0, 0), add=T)
...
# xlab('Alcohol level') +
# ylab('Freq Count') +
# ggtitle('Alcohol level distribution')
